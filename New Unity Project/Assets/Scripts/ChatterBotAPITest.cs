...
public class ChatterBotAPITest : MonoBehaviour {
	...
	public void Iniciar(string cadena){
		string url_1 = "https://www.cleverbot.com/getreply?key="+API_KEY_cleverbot.ToString()+"&input="+cadena;
		Debug.Log (url_1);
		WWW www = new WWW(url_1);
	...
	IEnumerator WaitForRequest(WWW www)
	{
		...
		if (www.error == null)
		{
			...
			text_output.text = "Bot dice: "+chatBotInfo.output;
			text_input.text = "Dices: "+chatBotInfo.input;

			//string res = "tts.readspeaker.com/a/speak?key="+API_KEY_speechcloud.ToString()+"&lang=es_es&voice=Pilar&text="+chatBotInfo.output+"&audioformat=ogg";
			//www = new WWW(res);
			//yield return www;
